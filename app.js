...
onAuthStateChanged(auth, (user) => {
    if (user) {
        // ** THIS IS THE FIX **
        // Call the function with NO parameters to match MainActivity.kt
        if (typeof Android !== "undefined" && Android.getFCMToken) {
            console.log("Requesting FCM token from Android...");
            Android.getFCMToken(); // <-- The call is now correct
        }
...
function initializeChatList(user) {
    ...
    unsubscribeFromContacts = onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            let badgeHtml = '';
            if (contact.unreadCount > 0) { badgeHtml = `<span class="unread-badge">${contact.unreadCount}</span>`; }
